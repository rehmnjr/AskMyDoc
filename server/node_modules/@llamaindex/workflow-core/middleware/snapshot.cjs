...
*/
const snapshotEvent = (0, __llamaindex_workflow_core.workflowEvent)();
	...
	const createSnapshotFn = (context) => {
		return async function snapshotHandler() {
			...
			const serializable = {
				...
				unrecoverableQueue: queue.filter((event) => !eventCounterWeakMap.has((0, __llamaindex_workflow_core.eventSource)(event))).map((event) => [event.data, getEventCounter((0, __llamaindex_workflow_core.eventSource)(event))]),
				version: getVersion(),
